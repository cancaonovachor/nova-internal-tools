...
2. Terraform apply (`notion_discord_bot/terraform/`) で env / secret version 変更を反映
3. **Cloud Run リビジョン強制が必要なケース** (後述) は `gcloud run deploy --image=<latest>` で push する

### 初回 apply のみ target 指定が必要

Cloud Run 作成時点でイメージが Artifact Registry に存在していないと失敗する。初回は:

```bash
# 1) APIs / AR / secrets / queue / SA を先行
terraform apply \
...
state backend は **GCS (`gs://starlit-road-203901-tfstate`, prefix `notion-discord-bot`)**。`terraform.tfvars` には secrets が入るので `.gitignore` 対象。
...
| `terraform/` | GCP 全リソース定義 (Cloud Run x2, Cloud Tasks, AR, Secret Manager, IAM, SA) |
